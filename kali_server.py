...
def execute_command(command: str, timeout: int = COMMAND_TIMEOUT) -> Dict[str, Any]:
    """
...
def generic_command():
    ...
    try:
        ...
        result = execute_command(command, timeout=int(timeout))
...
def nmap():
    ...
    try:
        ...
        timeout = params.get("timeout", 600)  # nmap gets a generous 10min default
...
def health_check():
    ...
    essential_tools = ["nmap", "gobuster", "dirb"]
    tools_status = {}
    ...
    return jsonify({
        "status": "healthy",
        "message": "HackBridge API Server is running",
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="HackBridge - Linux API Server for AI-Powered Offensive Security")
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting HackBridge API Server on {args.ip}:{API_PORT}")
