...
def setup_mcp_server(kali_client: KaliToolsClient) -> FastMCP:
    ...
    """
    mcp = FastMCP("hackbridge")
    
    @mcp.tool()
    def nmap_scan(target: str, scan_type: str = "-sV", ports: str = "", additional_args: str = "", timeout: int = 600) -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            timeout: Maximum time in seconds to wait for the scan to complete (default: 600)
    ...
    @mcp.tool()
    def execute_command(command: str, timeout: int = 600) -> Dict[str, Any]:
        """
        Execute an arbitrary command on the Kali server.
        
        Args:
            command: The command to execute
            timeout: Maximum time in seconds to wait for the command to complete (default: 600)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Run the HackBridge MCP Client")
...
def main():
    ...
    logger.info("Starting HackBridge MCP server")
    mcp.run()
